<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: google/ads/googleads/v7/errors/geo_target_constant_suggestion_error.proto

namespace Google\Ads\GoogleAds\V7\Errors;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\RepeatedField;
use Google\Protobuf\Internal\GPBUtil;

/**
 * Container for enum describing possible geo target constant suggestion errors.
 *
 * Generated from protobuf message <code>google.ads.googleads.v7.errors.GeoTargetConstantSuggestionErrorEnum</code>
 */
class GeoTargetConstantSuggestionErrorEnum extends \Google\Protobuf\Internal\Message
{

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     * }
     */
    public function __construct($data = NULL) {
        \GPBMetadata\Google\Ads\GoogleAds\V7\Errors\GeoTargetConstantSuggestionError::initOnce();
        parent::__construct($data);
    }

}

