...
use Google\Ads\GoogleAds\V15\Resources\ProductLink;
use Google\Ads\GoogleAds\V15\Services\CreateProductLinkResponse;
use Google\Ads\GoogleAds\V15\Services\ProductLinkServiceClient;
use Google\Ads\GoogleAds\V15\Services\RemoveProductLinkResponse;
...
class ProductLinkServiceClientTest extends GeneratedTest
{
    ...
    public function createProductLinkTest()
    {
        ...
        $response = $gapicClient->createProductLink($customerId, $productLink);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
        $actualRequestObject = $actualRequests[0]->getRequestObject();
        $this->assertSame('/google.ads.googleads.v15.services.ProductLinkService/CreateProductLink', $actualFuncCall);
    ...
    public function createProductLinkExceptionTest()
    {
        ...
        $productLink = new ProductLink();
        try {
            $gapicClient->createProductLink($customerId, $productLink);
    ...
    public function removeProductLinkTest()
    {
        ...
        $response = $gapicClient->removeProductLink($customerId, $formattedResourceName);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
        $actualRequestObject = $actualRequests[0]->getRequestObject();
        $this->assertSame('/google.ads.googleads.v15.services.ProductLinkService/RemoveProductLink', $actualFuncCall);
        $actualValue = $actualRequestObject->getCustomerId();
        $this->assertProtobufEquals($customerId, $actualValue);
    ...
    public function removeProductLinkExceptionTest()
    {
        ...
        $formattedResourceName = $gapicClient->productLinkName('[CUSTOMER_ID]', '[PRODUCT_LINK_ID]');
        try {
            $gapicClient->removeProductLink($customerId, $formattedResourceName);
            // If the $gapicClient method call did not throw, fail the test
            $this->fail('Expected an ApiException, but no exception was thrown.');
        } catch (ApiException $ex) {
            $this->assertEquals($status->code, $ex->getCode());
