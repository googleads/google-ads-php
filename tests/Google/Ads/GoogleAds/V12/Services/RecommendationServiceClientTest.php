...
use Google\Ads\GoogleAds\V12\Services\ApplyRecommendationResponse;
use Google\Ads\GoogleAds\V12\Services\DismissRecommendationResponse;
...
class RecommendationServiceClientTest extends GeneratedTest
{
    ...
    public function applyRecommendationTest()
    {
        ...
        $operations = [];
        $response = $gapicClient->applyRecommendation($customerId, $operations);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
        $actualRequestObject = $actualRequests[0]->getRequestObject();
        $this->assertSame('/google.ads.googleads.v12.services.RecommendationService/ApplyRecommendation', $actualFuncCall);
    ...
    public function applyRecommendationExceptionTest()
    {
        ...
        $operations = [];
        try {
            $gapicClient->applyRecommendation($customerId, $operations);
    ...
    public function dismissRecommendationTest()
    {
        ...
        $customerId = 'customerId-1772061412';
        $operations = [];
        $response = $gapicClient->dismissRecommendation($customerId, $operations);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
        $actualRequestObject = $actualRequests[0]->getRequestObject();
        $this->assertSame('/google.ads.googleads.v12.services.RecommendationService/DismissRecommendation', $actualFuncCall);
        $actualValue = $actualRequestObject->getCustomerId();
        $this->assertProtobufEquals($customerId, $actualValue);
        $actualValue = $actualRequestObject->getOperations();
        $this->assertProtobufEquals($operations, $actualValue);
    ...
    public function dismissRecommendationExceptionTest()
    {
        ...
        $operations = [];
        try {
            $gapicClient->dismissRecommendation($customerId, $operations);
            // If the $gapicClient method call did not throw, fail the test
            $this->fail('Expected an ApiException, but no exception was thrown.');
        } catch (ApiException $ex) {
            $this->assertEquals($status->code, $ex->getCode());
