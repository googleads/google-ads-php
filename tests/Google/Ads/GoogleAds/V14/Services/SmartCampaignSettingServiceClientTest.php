...
use Google\Ads\GoogleAds\V14\Services\GetSmartCampaignStatusResponse;
use Google\Ads\GoogleAds\V14\Services\MutateSmartCampaignSettingsResponse;
...
class SmartCampaignSettingServiceClientTest extends GeneratedTest
{
    ...
    public function getSmartCampaignStatusTest()
    {
        ...
        $response = $gapicClient->getSmartCampaignStatus($formattedResourceName);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
        $actualRequestObject = $actualRequests[0]->getRequestObject();
        $this->assertSame('/google.ads.googleads.v14.services.SmartCampaignSettingService/GetSmartCampaignStatus', $actualFuncCall);
    ...
    public function getSmartCampaignStatusExceptionTest()
    {
        ...
        $formattedResourceName = $gapicClient->smartCampaignSettingName('[CUSTOMER_ID]', '[CAMPAIGN_ID]');
        try {
            $gapicClient->getSmartCampaignStatus($formattedResourceName);
    ...
    public function mutateSmartCampaignSettingsTest()
    {
        ...
        $operations = [];
        $response = $gapicClient->mutateSmartCampaignSettings($customerId, $operations);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
        $actualRequestObject = $actualRequests[0]->getRequestObject();
        $this->assertSame('/google.ads.googleads.v14.services.SmartCampaignSettingService/MutateSmartCampaignSettings', $actualFuncCall);
    ...
    public function mutateSmartCampaignSettingsExceptionTest()
    {
        ...
        $operations = [];
        try {
            $gapicClient->mutateSmartCampaignSettings($customerId, $operations);
            // If the $gapicClient method call did not throw, fail the test
            $this->fail('Expected an ApiException, but no exception was thrown.');
        } catch (ApiException $ex) {
            $this->assertEquals($status->code, $ex->getCode());
